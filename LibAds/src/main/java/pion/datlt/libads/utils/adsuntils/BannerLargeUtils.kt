...
fun Fragment.showAdsBannerLarge(
    configName: String,
    ...
    if (checkConditionShowAds(context, configName)){
        //chinh lai ratio
        kotlin.runCatching {
            val viewGroupAdsParams = viewGroupAds.layoutParams as ViewGroup.LayoutParams
            val widthInDp = 320
            val heightInDp = 100
            val scale = context!!.resources.displayMetrics.density
            viewGroupAdsParams.width = (widthInDp * scale).toInt()
            viewGroupAdsParams.height = (heightInDp * scale).toInt()
